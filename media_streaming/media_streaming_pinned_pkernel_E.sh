...
filename="./results_pinned_kernel_E/media_streaming_pinned_0-${1}.out"
bash ../perf_scripts/perf_counters_1P.sh $filename $runtime ${cpulist} &
for i in $( seq 0 $1 )
do
    filename="./results_pinned_kernel_E/media_streaming_pinned_P_${cpulist}_${i}.out"
    bash ../perf_scripts/perf_counters_1P.sh $filename $runtime $i &
done
# Add another script for recording core 16
filename="./results_pinned_kernel_E/media_streaming_pinned_P_${cpulist}_16.out"
