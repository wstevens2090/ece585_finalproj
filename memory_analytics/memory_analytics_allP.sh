...
taskset --cpu-list 0-15 docker run --rm --volumes-from data cloudsuite3/in-memory-analytics \
    /data/ml-latest-small /data/myratings.csv & 
sleep 1

# Step 4: Record counters for 60s. Run with & to prevent blocking. 
bash ../perf_scripts/perf_counters_allP.sh ./results/memory_analytics_allP.out 10